...
/** @global Number of vertices Used for the Illini Shield */
var numCircleVerts = (24*3);

/** vextor of triangle verticies */
var triangleVertices; 

// Initialize the vector....
vec3.set(transformVec,0.0,0.0,-2.0);

//Used to count frames in non-affine transformation
var count = 0;
...
//helper value for non affine transformation
var sscalar = 0;
// variable to help bounce the orange part
var up = 1
...
function animate() {
    ...
    //If the count is less than 200, it runs through the Affine Transformation
    if (count < 180 || count > 2000){
        if (lastTime != 0) {
        ...
        /* Creates a new buffer and modifies the vertices based on count 
        */
        vertexPositionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, vertexPositionBuffer);
        
            for (var i = 0; i < triangleVertices.length; i++){
                if (triangleVertices[i+1] <= 0){
                    if (i % 24 >= 21){
